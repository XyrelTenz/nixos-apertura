...
python3 -u "$SCRIPT_DIR/get_ddg_links.py" "$QUERY" | while IFS='|' read -r thumb_url full_url; do
    ...
    # Validate image content type prior to downloading
    ...
    # Download thumbnail with a timeout to prevent hanging
    curl -s -L -m 5 -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" "$thumb_url" -o "$tmppath"

    # Re-check picker state post-download
    state=$(cat "$CONTROL_FILE" 2>/dev/null | tr -d '[:space:]')
    if [[ "$state" == "stop" ]]; then 
    ...
    # Ensure download is a valid image before saving
    if [ -s "$tmppath" ]; then
