...
Item {
    ...
    Component.onCompleted: {
        ...
        // Prepare local wallpaper thumbnails in background
        let prepScript = decodeURIComponent(Qt.resolvedUrl("wallpaper_prep.sh").toString());
        if (prepScript.startsWith("file://")) {
            prepScript = prepScript.substring(7);
        }
        Quickshell.execDetached(["bash", prepScript]);
